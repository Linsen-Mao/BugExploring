...
from bug_improving.utils.nlp_util import NLPUtil, SentUtil
...
class Step:
    """
    step includes attributes: id, bug, step, is_operation, prev_step, next_step,
    """

    def __init__(self, id, bug, text, is_operational=None, prev_step=None, next_step=None, cluster_index=None):
        self.id = id
        self.bug = bug
        self.text = text
    ...
    def __repr__(self):
        return f'{self.id} https://bugzilla.mozilla.org/show_bug.cgi?id={self.bug.id}\n ' \
               f'{self.text} - ' \
               f'({Placeholder.OPERATION}: {self.is_operational}, {Placeholder.STEP_CLUSTER}: {self.cluster_index}) ' \
               f'\n************\n'
    ...
    @classmethod
    def from_step(cls, id, bug, step, is_operational=None):
        return cls(id, bug, step, is_operational)

    def convert_step_into_step_dict(self):
        return {Placeholder.STEP: self.text, Placeholder.STEP_CLUSTER: self.cluster_index}
...
class Description:
    ...
    def from_text(cls, bug, text):
        ...
        if steps_to_reproduce:
            ...
            return cls(bug, text, prerequisites, steps_to_reproduce, expected_result, actual_result, notes)

        return cls(bug, text, prerequisites, None, expected_result, actual_result, notes)
    ...
    def get_step_list(self, with_step_cluster=False):
        steps = []
    ...
    def get_step_cluster_index_set(self):
        step_cluster_indexes = set()
        for step in self.steps_to_reproduce:
            step_cluster_indexes.add(step.cluster_index)
        return step_cluster_indexes

    def get_step_cluster_index_list(self):
        step_cluster_indexes = list()
        for step in self.steps_to_reproduce:
            step_cluster_indexes.append(step.cluster_index)
        return step_cluster_indexes

    # def chunk_steps_by_cluster_index(self, cluster_index):
    #     """
    #     @param cluster_index:
    #     @type cluster_index:
    #     @return:
    ...
    #     chunks = []
    #     step_indexes = []
    #     for index, step in enumerate(self.steps_to_reproduce):
    #         if step.cluster_index == cluster_index:
    #             step_indexes.append(index)
    #     for step_index in step_indexes:
    #         chunks.append((self.steps_to_reproduce[step_index],
    #                        self.steps_to_reproduce[0: step_index],
    #                        self.steps_to_reproduce[step_index+1:]))
    #
    #         # @todo: filter duplicate chunks by non-operation steps
    #         # chunks = self.filter_chunks_by_non_operation_step(chunks)
    #     return chunks

    def get_step_ids_by_cluster_index(self, cluster_index):
        """
        get step indexes (i.e., the ranking of the step in Steps-To-Reproduce: step.id) by cluster_index
        @param cluster_index:
        @type cluster_index:
        ...
        for index, step in enumerate(self.steps_to_reproduce):
            if step.cluster_index == cluster_index:
                # step_indexes.append(int(step.id))
                step_indexes.append(index)
    ...
    def filter_chunks_by_non_operation_step(self, chunks):
        """
        @todo: filter duplicate chunks by non-operation steps
        @param chunks:
        @type chunks:
    ...
    def get_sections_from_dict(self, section_dict):
        self.prerequisites = section_dict[Placeholder.PRECONDITIONS]
    ...
    def get_steps_to_reproduce_from_dict(self, step_dicts):
        steps = []
        for index, step_dict in enumerate(step_dicts):
            is_operational = False
            try:
                if step_dict[Placeholder.STEP_TYPE] == Placeholder.OPERATION:
                    is_operational = True
                elif step_dict[Placeholder.STEP_TYPE] == Placeholder.NON_OPERATION:
                    is_operational = False
                ...
                pass
            try:
                step = Step(index, self.bug, step_dict[Placeholder.STEP], is_operational)
            except Exception as e:
                print(self.bug)
                print(f"{e}")
                print(step_dict)
                step = Step(index, self.bug, "", is_operational)
                pass
            steps.append(step)

        for index, step in enumerate(steps):
            if index != 0:
                step.prev_step = steps[index - 1]
